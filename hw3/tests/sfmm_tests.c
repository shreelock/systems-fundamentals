...
Test(sf_memsuite_student, realloc_same_size, .init = sf_mem_init, .fini = sf_mem_fini) {
    double* ptr = sf_malloc(34);*ptr=20;
    double* nptr = sf_realloc(ptr, 34);
    cr_assert(ptr==nptr, "Same size should return same pointers");
    cr_assert(*ptr==*nptr,"values should be same after realloc");
}

Test(sf_memsuite_student, realloc_same_block_size, .init = sf_mem_init, .fini = sf_mem_fini) {
    int ip_req_size = 10;
    int op_req_size = 12;
    double* ptr = sf_malloc(ip_req_size);*ptr=20;
    double* nptr = sf_realloc(ptr, op_req_size);

    cr_assert(ptr==nptr, "Same block size should return same pointers");
    cr_assert(*ptr==*nptr,"values should be same after realloc with same block size");

    sf_header* h1 = (void*)ptr - sizeof(sf_header);
    sf_footer* f1 = (void*)h1 + (h1->block_size<<4) - sizeof(sf_footer);

    sf_header* h2 = (void*)nptr - sizeof(sf_header);
    sf_footer* f2 = (void*)h2 + (h2->block_size<<4) - sizeof(sf_footer);

    cr_assert(f2->block_size==f1->block_size,"Block size entries must be same");
    cr_assert(f2->requested_size != ip_req_size,"requested size should not be same as ip size");
    cr_assert(f2->requested_size == op_req_size,"requested size should be changed to op size");
}

Test(sf_memsuite_student, realloc_higher_block_size, .init = sf_mem_init, .fini = sf_mem_fini) {
    int ip_val = 20, ip_size = 10, op_size = 20, sizeoffirstfreelist_now=0;
    double* ptr = sf_malloc(ip_size);*ptr=ip_val;
    double* nptr = sf_realloc(ptr, op_size);
    for (int i = 0; i < FREE_LIST_COUNT; i++) {
        sf_free_header *fh = (sf_free_header *) (seg_free_list[i].head);
        if (fh != NULL){
            sizeoffirstfreelist_now = seg_free_list[i].head->header.block_size<<4;
            break;
        }
    }

    cr_assert(*nptr==ip_val,"Values not same");
    cr_assert(sizeoffirstfreelist_now == 32,"Old pointer must be coalesced back to free list");
}

Test(sf_memsuite_student, realloc_smaller_block_size, .init = sf_mem_init, .fini = sf_mem_fini) {
    int ip_val = 20, ip_size = 65, op_size = 32, sizeoffirstfreelist_now = 0, sizeoffirstfreelist_then = 0;

    double* ptr = sf_malloc(ip_size);*ptr=ip_val;
    for (int i = 0; i < FREE_LIST_COUNT; i++) {
        sf_free_header *fh = (sf_free_header *) (seg_free_list[i].head);
        if (fh != NULL){
            sizeoffirstfreelist_then = seg_free_list[i].head->header.block_size<<4;
            break;
        }
    }
    ...
    cr_assert(*nptr==ip_val,"Values not same");
    cr_assert(sizeoffirstfreelist_now-sizeoffirstfreelist_then == 48,"Old pointer must be coalesced back to free list");
}